...
class DeepInfraClient(LLMClient):
    def __init__(self, api_key: str | None, base_url: str, model: str, timeout: float) -> None:
        super().__init__(provider="deepinfra", model=model)
        if not api_key:
    ...
    async def generate(self, prompt: str) -> str:
        ...
        if not choices:
            raise RuntimeError("DeepInfra response missing choices")
        message = choices[0].get("message") or {}
        content = message.get("content")
        if not content:
            raise RuntimeError("DeepInfra response missing content")
        return content
